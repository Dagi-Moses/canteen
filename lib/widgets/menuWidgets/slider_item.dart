...
class SliderItem extends StatelessWidget {
  ...
  SliderItem({
    ...
    required this.model,
    required this.menuProvider, this.onTap
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      behavior: HitTestBehavior.translucent,
         onTap: onTap ??
          () {
            // Default behavior if no function is passed
            Navigator.pushNamed(
              context,
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    if (model.rating != 0 || model.raters.isNotEmpty)
                      Text(
                        "${model.rating % 1 == 0 ? model.rating.toStringAsFixed(0) : model.rating.toStringAsFixed(1)} (${model.raters.length.toString()} )",
                        style: const TextStyle(
                          fontSize: 16.0,
                        ),
                      )
